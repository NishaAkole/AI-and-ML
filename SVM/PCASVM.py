...
plot_decision_regions(X, y.astype(np.integer),svc_lin, legend=2)
...
plt.scatter(X[:int(len(X)/2),0],X[:int(len(X)/2),1],c='r',alpha=0.3,label="Target Present")
plt.scatter(X[int(len(X)/2):,0],X[int(len(X)/2):,1],c='b',alpha=0.3,label="Target Absent")
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
